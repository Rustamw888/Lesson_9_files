...
import guru.qa.homework.hwDomain.Team;
import guru.qa.lesson.domain.User;
...
public class FilesParsingTests {
    ...
    @Test
    public void downloadTextFormatFile() throws Exception {
        open("https://filesamples.com/formats/txt");

        File txtPath = $("a[href*='sample3']").download();
        // стандартный способ чтения текстового файла
        try (InputStream is = new FileInputStream(txtPath)) {
            assertThat(new String(is.readAllBytes(), UTF_8)).contains("Idemne, quod iucunde?");
        }
        // альтернативный способ чтения текстового файла
        String alternative = Files.readString(txtPath.toPath(), UTF_8);
        assertThat(alternative).contains("Idemne, quod iucunde?");

        open("https://the-internet.herokuapp.com/upload");

        $("input[type='file']").uploadFile(txtPath);
        $("#file-submit").click();
        $("div.example").shouldHave(text("File Uploaded!"));
        $("#uploaded-files").shouldHave(text("sample3.txt"));
    }
...
//        ClassLoader classLoader1 = FilesParsingTests.class.getClassLoader(); // можно использовать в статических методах
        try (InputStream is = classLoader.getResourceAsStream("files/Machine_readable_file_bdc_sf_2021_q4.csv");
             CSVReader reader = new CSVReader(new InputStreamReader(is))) { // можем использовать несколько выражений в блоке", " для автоматического закрытия", " когда станут не нужны
    ...
    @Test
    public void jsonJacksonCommonTest() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        try (InputStream is = classLoader.getResourceAsStream("files/jackson.json")) {
            String json = (new String(is.readAllBytes(), UTF_8));
    ...
    @Test
    public void jsonJacksonTypeTest() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        try (InputStream is = classLoader.getResourceAsStream("files/jackson.json")) {
            String json = (new String(is.readAllBytes(), UTF_8));
            JsonNode jsonNode = objectMapper.readTree(json);
